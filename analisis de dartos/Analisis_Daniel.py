...
df_q1["bilingue_label"] = df_q1["cole_bilingue"].map({"S": "Bilingüe", "N": "No bilingüe"}).fillna(df_q1["cole_bilingue"])
df_q1["estrato_label"] = df_q1["fami_estratovivienda"].astype(str)
...
df_q1["internet_label"] = df_q1["fami_tieneinternet"].apply(_si_no)
df_q1["computador_label"] = df_q1["fami_tienecomputador"].apply(_si_no)

def _acceso(row):
    i = row["internet_label"]
    c = row["computador_label"]
    if i == "Si" and c == "Si":
...
df_q1["acceso_digital"] = df_q1.apply(_acceso, axis=1)

estrato_order = ["SIN ESTRATO", "ESTRATO 1", "ESTRATO 2", "ESTRATO 3", "ESTRATO 4", "ESTRATO 5", "ESTRATO 6"]
estrato_order = [e for e in estrato_order if e in set(df_q1["estrato_label"])]
if len(estrato_order) == 0:
    estrato_order = sorted(df_q1["estrato_label"].unique().tolist())

category_orders = {
    "bilingue_label": ["No bilingüe", "Bilingüe"],
    "estrato_label": estrato_order,
    "internet_label": ["No", "Si"],
    "computador_label": ["No", "Si"],
    "acceso_digital": ["Sin internet y sin computador", "Solo internet", "Solo computador", "Internet y computador"],
}

salida = Path("figuras_q1")
salida.mkdir(exist_ok=True)

def _mostrar_y_guardar(fig, nombre_archivo):
    fig.update_layout(template="plotly_white", font=dict(size=14), title_x=0.02)
    fig.write_html(salida / nombre_archivo, include_plotlyjs="cdn")
    fig.show()
...
fig1_q1 = px.box(
    ...
    x="estrato_label",
    y="punt_global",
    color="bilingue_label",
    points=False,
    category_orders=category_orders,
    labels={"estrato_label": "Estrato de la vivienda", "punt_global": "Puntaje global", "bilingue_label": "Colegio"},
    title="Puntaje global por estrato comparando colegios bilingües y no bilingües"
)
fig1_q1.update_layout(boxmode="group")
_mostrar_y_guardar(fig1_q1, "02_box_estrato_bilingue.html")

medianas = (
    df_q1.groupby(["estrato_label", "acceso_digital", "bilingue_label"], as_index=False)["punt_global"]
    .median()
)

pivot = medianas.pivot_table(
    index=["estrato_label"],
    columns=["acceso_digital", "bilingue_label"],
    values="punt_global",
    aggfunc="median"
)

diff = (
    pivot.xs("Bilingüe", axis=1, level=1) -
    pivot.xs("No bilingüe", axis=1, level=1)
).reindex(index=estrato_order).reindex(columns=category_orders["acceso_digital"])
...
fig2_q1 = px.imshow(
    ...
    title="Ventaja bilingüe por estrato y acceso digital (mediana Bilingüe menos No bilingüe)"
)
_mostrar_y_guardar(fig2_q1, "04_heatmap_ventaja_bilingue.html")

df_global["bilingue_label"] = df_global["cole_bilingue"].map({"S": "Bilingüe", "N": "No bilingüe"}).fillna(df_global["cole_bilingue"])
n_sample = 8000
df_sm = df_global.sample(n=min(len(df_global), n_sample), random_state=42)
...
labels_sm = {
    ...
    "punt_global": "Global",
    "bilingue_label": "Colegio",
}

fig3_q1 = px.scatter_matrix(
    df_sm,
    dimensions=[
        "punt_ingles",
        "punt_matematicas",
    ...
    opacity=0.55,
    labels=labels_sm,
    title="Matriz de dispersión de los 5 puntajes y el puntaje global, comparando bilingüismo"
)

fig3_q1.update_traces(diagonal_visible=False, marker=dict(size=4))
fig3_q1.update_layout(margin=dict(l=140, r=40, t=80, b=60), height=750, template="plotly_white", font=dict(size=13), title_x=0.02)
fig3_q1.update_xaxes(automargin=True)
fig3_q1.update_yaxes(automargin=True)

_mostrar_y_guardar(fig3_q1, "08_scatter_matrix_puntajes_vs_global_sin_estrato.html")